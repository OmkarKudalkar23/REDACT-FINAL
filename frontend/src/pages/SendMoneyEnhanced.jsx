...
const SendMoneyEnhanced = () => {
  ...
  useGSAP(() => {
    ...
    tl.from("#send-money-video-frame", {
      clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)",
      borderRadius: "0% 0% 0% 0%",
      duration: 1.2,
      ease: "power1.inOut",
    })
      .from(".send-money-hero-content", {
        opacity: 0,
        y: 50,
        duration: 0.8,
        ease: "power2.out",
      }, "-=0.6")
      .from(".send-money-card", {
        opacity: 0,
        y: 30,
        duration: 0.6,
        stagger: 0.1,
    ...
    gsap.set("#send-money-video-frame", {
  ...
  return (
    <div className="relative min-h-screen w-screen overflow-x-hidden bg-black">
      ...
      <div className="relative h-screen w-screen flex items-center justify-center px-5 sm:px-10 pt-20 overflow-hidden">
        ...
        {/* Hero Content */}
        <div className="send-money-hero-content relative z-10 text-center max-w-3xl">
          ...
          <div className="flex flex-wrap justify-center gap-4 text-sm font-circular-web">
            <div className="px-4 py-2 bg-blue-900/20 rounded-full text-blue-200 backdrop-blur-sm">⚡ Instant Transfer</div>
            <div className="px-4 py-2 bg-blue-900/20 rounded-full text-blue-200 backdrop-blur-sm">🔒 Secure & Encrypted</div>
            <div className="px-4 py-2 bg-blue-900/20 rounded-full text-blue-200 backdrop-blur-sm">💰 Zero Fees</div>
          </div>
        </div>
      </div>
      ...
      <section className="relative py-20 px-5 sm:px-10 border-t border-blue-900/30">
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { icon: "⚡", title: "Instant", description: "Money arrives in seconds", video: 0 },
              { icon: "🔒", title: "Secure", description: "Bank-level encryption", video: 1 },
              { icon: "💰", title: "No Fees", description: "Zero hidden charges", video: 2 },
              { icon: "🌍", title: "Global", description: "Send anywhere worldwide", video: 3 },
            ].map((benefit, idx) => (
              <div
                key={idx}
                className="send-money-card group relative overflow-hidden border-hsla rounded-lg p-6 bg-gradient-to-br from-blue-900/20 to-purple-900/20 backdrop-blur-sm hover:from-blue-900/40 hover:to-purple-900/40 transition-all duration-300 hover:scale-105"
              >
                {/* Video Background */}
                <video
      ...
      <section className="relative py-20 px-5 sm:px-10">
        <div className="container mx-auto max-w-4xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {recipients.map((recipient) => (
                    <button
                      ...
                      className="send-money-card group relative overflow-hidden border-hsla rounded-lg p-6 bg-gradient-to-br from-blue-900/20 to-purple-900/20 backdrop-blur-sm hover:from-blue-900/40 hover:to-purple-900/40 transition-all duration-300 text-left hover:scale-105"
                    >
                      {/* Video Background */}
                      <video
                        src={`/videos/feature-${(recipient.id % 5) + 1}.mp4`}
                        autoPlay
                        muted
                        loop
          ...
          {step === 2 && selectedRecipient && (
            <div className="space-y-8">
              {/* Recipient Summary */}
              <div className="send-money-card group relative overflow-hidden border-hsla rounded-lg p-8 bg-gradient-to-br from-blue-900/20 to-purple-900/20 backdrop-blur-sm">
                {/* Video Background */}
                <video
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[50, 100, 500, 1000].map((quickAmount) => (
                    <button
                      ...
                      className="send-money-card border-hsla rounded-lg py-4 md:py-5 px-4 bg-blue-900/20 hover:bg-yellow-300 hover:text-black text-blue-50 font-bold text-base md:text-lg transition-all duration-300 hover:scale-105 border-2 border-blue-900/30 hover:border-yellow-300"
                    >
                      ${quickAmount}
                    </button>
                  ))}
              ...
              <div className="flex gap-4 pt-8">
                <button
                  onClick={() => setStep(1)}
                  className="flex-1 border-hsla border-2 rounded-lg py-4 md:py-5 px-6 bg-blue-900/20 hover:bg-blue-900/40 text-blue-50 font-bold text-base md:text-lg transition-all duration-300 hover:scale-105 hover:border-yellow-300"
                >
                  Back
                </button>
                <button
                  ...
                  disabled={!amount}
                  className="flex-1 rounded-lg py-4 md:py-5 px-6 bg-yellow-300 hover:bg-yellow-400 disabled:opacity-50 disabled:cursor-not-allowed text-black font-bold text-base md:text-lg transition-all duration-300 hover:scale-105 flex items-center justify-center gap-2"
                >
...
export default SendMoneyEnhanced;
